...
class apiFIPE:
    ...
    def _get_codigo_tabela_referencia(self):
        ...
        try:
            ...
            raise FipeApiError(
                "Could not parse reference table response. API may have changed."
            )
    ...
    def get_valor_por_codigo_fipe(self, ano_modelo, codigo_fipe):
        ...
        try:
            response = self.session.post(
                f"{self.BASE_URL}/ConsultarValorComTodosParametros", data=data
            )
            response.raise_for_status()
            ...
            if (
                "erro" in json_response
                or "codigo" in json_response
                and json_response.get("Valor") is None
            ):
                ...
                _logger.warning(
                    "FIPE API returned an error for FIPE code %s: %s",
                    codigo_fipe,
                    error_msg,
                )
            ...
            raise FipeApiError(
                f"Network error fetching vehicle value by FIPE code: {e}"
            )
        except ValueError:  # Catches JSON decoding errors
            raise FipeApiError(
                "Failed to decode API response. The API might be down or has changed."
            )
