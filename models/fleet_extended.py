...
class FleetOperations(models.Model):
    ...
    def _compute_result_apifipe(self):
        ...
        for record in self:
            if record.model_year and record.fipe_id:
                result_apifipe = apifipecons.apiFIPE().getCodigoMarca(record.model_year, record.fipe_id)
                record.result_apifipe = result_apifipe
                record.resale_value = float(result_apifipe['Valor'][3:].replace(".", "").replace(",", "."))
    ...
    def _compute_resale_value(self):
        ...
        for record in self:
            if record.result_apifipe:
                record.resale_value = float(eval(record.result_apifipe)['Valor'][3:].replace(".", "").replace(",", "."))
            else:
                record.resale_value = record.resale_value

    fuel_type = fields.Selection(selection_add=[('flex', 'Flex')],
                                 string='Fuel Type',
                                 default='flex',
                                 help='Fuel Used by the vehicle')

    model_year = fields.Selection([(str(num), str(num)) for num in range( (int(datetime.now().year)-10), (int(datetime.now().year)+1) )],
                help="Vehicle year.")
    ...
    fipe_model = fields.Char(string='Modelo Tabela FIPE',
                #compute='_compute_fipe_model'
                compute='_compute_result_apifipe'
                )

    result_apifipe = fields.Char(string='Resultado consulta FIPE',
                compute='_compute_result_apifipe'
                )
